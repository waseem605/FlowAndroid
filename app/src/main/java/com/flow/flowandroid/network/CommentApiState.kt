...
data class CommentApiState<out WeatherMainModel>(val status: Status, val data:WeatherMainModel?, val message: String?) {

    companion object {

        // In case of Success,set status as
        // Success and data as the response
        fun successData(data: WeatherMainModel?): CommentApiState<WeatherMainModel> {
            return CommentApiState(Status.SUCCESS, data, "null")
        }
